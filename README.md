...
- 一般メンバーは他者の個人別出社予定を閲覧できません。今日の利用予定は人数・開錠予定の集計のみで、自分の予約は今後分を閲覧できます。このDB制御は002の適用が必要です。
...
`CHATGPT_HANDOFF.md` を参照してください。上記の初期MVP手順に対し、現在のコードは002・003の追加SQLとmember/key_manager/adminロールを前提とします。実DBでの追加SQL適用は未確認です。無効なProject URLではログイン画面に設定エラーを表示し、認証操作を停止します。正しい接続設定の代用にはなりません。ローカル起動は `npm run dev -- --port 3001` です。
